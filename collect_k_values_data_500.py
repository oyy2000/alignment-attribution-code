...
def collect_data_for_sparsity(sparsity_ratio: float) -> Dict:
    ...
    # File paths
    cot_goldreason_file = "out/llama2-7b-chat-hf/unstructured/wanda_weightonly/GSM8K_direct_120/sparsity_0/nsamples_500/gsm8k_bottom_0.000000_cot0shot_goldreason_held_out.jsonl"
    direct_file = "out/llama2-7b-chat-hf/unstructured/wanda_weightonly/GSM8K_direct_120/sparsity_0/nsamples_500/gsm8k_bottom_0.000000_direct_held_out.jsonl"
    
    # Find the directory that contains files with the target sparsity ratio
    base_dir = "out/llama2-7b-chat-hf/unstructured/wanda_3_set_difference_weightonly/GSM8K_cot0shot_120/prompt_direct,cot0shot,cot0shot_goldreason/"
...
def plot_results(all_results: Dict[float, Dict]):
    ...
    print("SUMMARY TABLE")
    print("="*80)
    print(f"{'Sparsity':<12} {'Category':<25} {'Original':<10} {'Pruned':<10} {'Change':<10} {'Count':<8}")
    print("-"*80)
    ...
    for sparsity_ratio in sparsity_ratios:
        for category in categories:
            if category in all_results[sparsity_ratio]:
                result = all_results[sparsity_ratio][category]
                print(f"{sparsity_ratio:<12} {category:<25} {result['original_accuracy']:<10.3f} {result['pruned_accuracy']:<10.3f} {result['accuracy_change']:<10.3f} {result['count']:<8}")
        ...
        try:
            ...
            for category in ["direct_success_cot_success", "direct_fail_cot_success"]:
                if category in results:
                    ...
                    print(f"    {category}: {result['count']} samples, "
