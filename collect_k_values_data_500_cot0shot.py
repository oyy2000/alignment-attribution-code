...
def collect_data_for_sparsity(sparsity_ratio: float) -> Dict:
    ...
    pruned_cot_goldreason_prompt_file = f"{target_sparsity_dir}gsm8k_bottom_{target_sparsity_value}_direct,cot0shot,cot0shot_goldreason_held_out_prompt_cot0shot.jsonl"
    ...
    # Load data
    cot0shot_data = load_jsonl(cot0shot_file)
    ...
    cot0shot_info = extract_sample_info(cot0shot_data)
    ...
    for name, samples in category_samples.items():
        analysis = analyze_category_performance(name, samples, cot0shot_info, direct_info, 
