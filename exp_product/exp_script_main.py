...
prompt_methods = ["direct,cot0shot,cot4shot"] #["cot2shot,cot4shot,cot8shot,cot16shot"]
# pq_options = [0.01]  # (p, q) for 3-set pruning
pq_options = [round(0.01 * i, 2) for i in range(1, 50)]   # 0.01 → 0.10
k_options = [0.17]
u_options = [0.15]
sparsity_threshold = 0.0000002
eval_dataset = "Product:3"
nsamples = 600
eval_type = "all"
prune_data = "gsm8k"
out_dir = "/mnt/beegfs/youyang7/projects/alignment-attribution-code/out"
log_file = f"command_log_[round(0.01 * i, 2) for i in range(1, 50)]_step_0.01_sp_{sparsity_threshold}.json"
def build_command(prune_method, prompt_method, p, q, k, u):
    save_dir = f"/mnt/beegfs/youyang7/projects/alignment-attribution-code/out/{eval_dataset}/{model}/{sparsity_type}/{prune_method}_{suffix}/wanda_3_set_difference_cot0shot/eval_{eval_type}/prompt_{prompt_method}/step_0.01_sp_{sparsity_threshold}_k_0.01/pq_{p}_{q}_k_{k}_u_{u}/"
    command = (
        f"python ../main.py "
