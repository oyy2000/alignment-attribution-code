...
k_options = [0.17] 
u_options = [0.15]
log_file = f"command_log_eval_gsm8k_wanda_4_set_454_alpaca_cleaned_no_safety_pquk_grid_search_[round(0.1 * i, 2) for i in range(1, 50)] _0.01.json"
def build_command(prune_method, prompt_method, p, q, k, u):
    save_dir = f"out/{model}/{sparsity_type}/{prune_method}_{suffix}/wanda_4_set_difference_cot0shot/eval_{eval_type}/prompt_{prompt_method}/pure_pq_0.01_granular/pq_{p}_{q}_k_{k}_u_{u}/"
    command = (
        f"python main.py "
        f"--model {model} "
