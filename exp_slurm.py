...
# ========================
BASE_DIR = "/home/youyang7/projects/alignment-attribution-code"
...
def build_command(prune_method, prompt_method, p, q, k, u):
    save_dir = os.path.join(
        ...
        f"4_set_alpaca_cleaned_no_safety/eval_{eval_type}/"
        f"prompt_{prompt_method}/pq_{p}_{q}_k_{k}_u_{u}_granular/"
    )
    cache_dir = os.path.join(OUT_DIR, "llm_weights")

    command = (
        f"python {os.path.join(BASE_DIR, 'main_hpc.py')} "
        f"--model {model} "
